...
protected:
    /**
     * @brief Represents a tile, that is a tuple of posit and u-bit.
     *
     * Only used for valid-internal plumbing.
     */
    struct tile
    {
        posit_type p;
     ...
     * multiplication), we have to fix the rounding to include the full range.
     * For example, computing 1 + 1 on the following primitive posit type
     *
     *    <---  |  ---   |  ---  |  ---  |  --->
     *          0       1/4      1       4
     *
     * the result of posit arithmetic would be 1 + 1 = 1 wit the result
     * rounded down. If 1 + 1 is supposed to be a left bound of some result,
     * rounding down is okay as the resulting valid will contain the exact but
     * not representable result 2. However, if 1 + 1 is a right bound, we need
     * to undo the rounding and round back up such that the exact result 2 is
     * part of the resulting valid.
     *
     * This function does this fixing.
     *
     * @param value Posit result of a given arithmetic operation.
     * @param rvalue Rounding that occurred during execution of the arithmetic operation.
     * @param is_left Whether we are computing a left bound.
     */
    [[nodiscard]] static constexpr tile adapt(const posit_type& value, const rounding_event rvalue,
                                              const interval_bound desired, bool is_left);

    /**
     * @brief Fix rounding of left endpoint in arithmetic result.
     */
    [[nodiscard]] static constexpr tile
    adapt_left(const posit_type& value, const rounding_event rvalue, const interval_bound desired);

    /**
     * @brief Fix rounding of right endpoint in arithmetic result.
     *
