...
auto to_base_2n(const normfloat<E, M> nf)
-> std::string
{
    std::string str;
...
auto to_hex(const normfloat<E, M>& value) -> std::string
{
    return to_base_2n<4>(value);
}
...
template<size_t E, size_t M>
auto to_octal(const normfloat<E, M>& value) -> std::string
{
    return to_base_2n<3>(value);
}
...
template<size_t E, size_t M>
auto to_binary(const normfloat<E, M>& value) -> std::string
{
...
template<size_t E, size_t M>
auto to_compute_string(const normfloat<E, M> nf)
-> std::string
{
    ...
    if(ones < zeroes)
    {
        for(auto counter = E + M; counter > M; --counter)
        {
            stream << ((nf.bit(counter-1) == 1 && !first)?" + ":"") << ((nf.bit(counter-1) == 1)?(("2^") + std::to_string(counter-M-1)):"");
    ...
    else
    {
        for(auto counter = E + M; counter > M; --counter)
        {
            stream << ((nf.bit(counter-1) == 0 && !first)?" - ":"") << ((nf.bit(counter-1) == 1)?((counter == E + M)?(("2^") + std::to_string(counter-M-1)):""):((counter == E+M)?("0"):("2^" + std::to_string(counter-M-1))));
            first = false;
        }
    }
...
template<size_t E, size_t M>
auto to_sci_string(const normfloat<E, M> nf)
-> std::string
{
    ...
    str << ((nf.get_sign() == 1)?"-":"")
        << *mantissa << "E" << (exponent.bit(E-1) == 1?"-":"")
        << to_decimal(abs_exponent);
