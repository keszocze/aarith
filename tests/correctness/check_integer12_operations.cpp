...
template <typename WordType> void helper()
{
    using namespace std;
    ...
    check_int_operation<integer, Width, B, false, WordType>(
        "mul_naive", &::aarith::naive_mul<Width, WordType>, native_mul);
    check_int_operation<integer, Width, B, false, WordType>(
        "mul_inplace", &::aarith::booth_inplace_mul<Width, WordType>, native_mul);
