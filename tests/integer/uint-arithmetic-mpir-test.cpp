...
bool is_uint_add_correct(const uinteger<BitWidth, WordType>& lhs,
                         const uinteger<BitWidth, WordType>& rhs,
...
template <size_t BitWidth, typename WordType>
bool is_uint_mul_correct(const uinteger<BitWidth, WordType>& lhs,
                         const uinteger<BitWidth, WordType>& rhs,
                         const uinteger<BitWidth, WordType>& result)
{
    mpz_t a_mpir, b_mpir, result_mpir, mask_mpir;
...
{
    auto const lhs = GENERATE(take(4, random_uinteger<BitWidth, WordType>()));
    auto const rhs = GENERATE(take(4, random_uinteger<BitWidth, WordType>()));
                       ...
                       AARITH_INT_TEST_TEMPLATE_LARGE_PARAM_RANGE)
{
    ...
    const I lhs = GENERATE(take(4, random_uinteger<W, WordType>()));
    const I rhs = GENERATE(take(4, random_uinteger<W, WordType>()));
...
//                       "[integer][signed][arithmetic][multiplication][mpir]",
//                       AARITH_INT_TEST_SIGNATURE, AARITH_INT_TEST_TEMPLATE_LARGE_PARAM_RANGE)
//{
//    using I = uinteger<W, WordType>;
//    const I lhs =
